...
class Trader:
    ...
    def _init_exchange(self):
        ...
        try:
            ...
            logger.info("[STEP 1/5] Testing Bybit API connectivity...")
            try:
                ...
                if response.status_code != 200:
                    logger.warning(f"  ⚠ Bybit API returned {response.status_code} - may indicate IP blocking")
            except requests.exceptions.Timeout:
                logger.error("  ✗ TIMEOUT reaching Bybit API - server not responding")
            except requests.exceptions.ConnectionError:
                logger.error("  ✗ CONNECTION ERROR - cannot reach Bybit API (IP may be blocked)")
            ...
            logger.info("[STEP 4/5] Loading markets (3 retries)...")
