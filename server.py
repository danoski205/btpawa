        ...
        try:
            ...
            if spot_price > self.last_buy_level + GRID_SIZE:
                ...
                if base_balance < TRADE_AMOUNT:
                    logger.warning(f"Insufficient {BASE_ASSET} balance: {base_balance} < {TRADE_AMOUNT}")
                    return
                
                # CRITICAL FIX #2: Hedge-first with emergency close on failure
                perp_order = await self.execute_perp_long(TRADE_AMOUNT)
                if not perp_order:
                    logger.error("Perp hedge failed - aborting sell to avoid naked position")
                    return
                ...
                if not spot_order:
                    # EMERGENCY: Spot sell failed but perp long opened → close perp immediately
                    logger.warning("Spot sell failed - closing perp hedge to prevent naked long position")
                    await self.execute_perp_short(TRADE_AMOUNT)
                    return
                
                # CRITICAL FIX #3: Real profit calculation with actual fees
                spot_fee = TRADE_AMOUNT * spot_price * 0.001  # 0.1% taker
                perp_fee = TRADE_AMOUNT * perp_price * 0.0006  # 0.06% taker
                gross_profit = (spot_price - self.last_buy_level) * TRADE_AMOUNT
                net_profit = gross_profit - spot_fee - perp_fee
                profit_percent = (net_profit / (self.last_buy_level * TRADE_AMOUNT)) * 100
                ...
                if profit_percent > 0:
                    ...
                    logger.info(f"[TRADE] Sell profitable: {profit_percent:.4f}% (gross: {(gross_profit/(self.last_buy_level*TRADE_AMOUNT)*100):.4f}%)")
                    self.last_sell_level = spot_price
                else:
                    logger.info(f"[TRADE] Sell not profitable: {profit_percent:.4f}% (skipped)")
            
            # If price dropped below grid, execute buy
            elif spot_price < self.last_sell_level - GRID_SIZE:
                logger.info(f"[GRID] Price {spot_price} < threshold {self.last_sell_level - GRID_SIZE} → BUY")
                
                # Check balance before trading
                balance = self.spot.fetch_balance()
                usdt_balance = balance.get(QUOTE_ASSET, {}).get("free", 0)
                required_usdt = TRADE_AMOUNT * spot_price
                if usdt_balance < required_usdt:
                    logger.warning(f"Insufficient {QUOTE_ASSET} balance: {usdt_balance} < {required_usdt}")
                    return
                
                # CRITICAL FIX #2: Hedge-first with emergency close on failure
                perp_order = await self.execute_perp_short(TRADE_AMOUNT)
                if not perp_order:
                    logger.error("Perp hedge failed - aborting buy to avoid naked position")
                    return
                ...
                if not spot_order:
                    # EMERGENCY: Spot buy failed but perp short opened → close perp immediately
                    logger.warning("Spot buy failed - closing perp hedge to prevent naked short position")
                    await self.execute_perp_long(TRADE_AMOUNT)
                    return
                
                # Track buy for future profit calculation when we sell
                spot_fee = TRADE_AMOUNT * spot_price * 0.001  # 0.1% taker
                perp_fee = TRADE_AMOUNT * spot_price * 0.0006  # 0.06% taker (approx)
                ...
                state.add_trade(Trade(
                    ...
                    amount=TRADE_AMOUNT,
                    fee=total_fee,
                    profit_percent=0.0,  # Will be calculated when sold
                    hedge_type="spot+perp"
                ))
                logger.info(f"[TRADE] Buy executed at {spot_price} (fees: {total_fee:.8f})")
                self.last_buy_level = spot_price
