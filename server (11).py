...
class ArbitrageScanner:
    """
    High-performance arbitrage scanner with caching and real-time price updates.
    
    Strategy:
    1. Load all pairs once at startup (cached)
    2. Update prices in real-time via fast polling
    3. Check triangles instantly against cached data
    4. No rescanning - just price updates
    """

    def __init__(self, exchange, min_profit_percent: float = 0.6):
        self.exchange = exchange
        self.min_profit_percent = min_profit_percent
        
        # Pair cache: symbol -> PairInfo
        self.pair_cache: Dict[str, PairInfo] = {}
        
        # Price cache: symbol -> {"bid": x, "ask": y, "last_update": timestamp}
        self.price_cache: Dict[str, dict] = {}
        
        # Metadata: all symbols and asset mappings
        self.all_symbols: List[str] = []
        self.usdt_pairs: List[str] = []
        self.assets: set = set()
    ...
    async def initialize_cache(self):
        """
        Load top 40 liquid pairs and build cache.
        Uses fetch_tickers() to get ALL prices in ONE call (~400ms).
        """
        try:
            logger.info("[Scanner CACHE] Initializing cache with TOP 40 LIQUID pairs...")
            
            self.all_symbols = self.exchange.symbols
            self.usdt_pairs = [s for s in self.all_symbols if "/USDT" in s]
            
            # CRITICAL FIX: Only use top 40 pairs (high liquidity, real arbitrage)
            # Small coins have wide spreads and low depth - arbitrage impossible
            self.usdt_pairs = self.usdt_pairs[:40]
            
            logger.info(f"[Scanner CACHE] Focusing on top {len(self.usdt_pairs)} liquid pairs")
            
            # Extract all unique assets from these pairs
            for symbol in self.usdt_pairs:
                base = symbol.split("/")[0]
                self.assets.add(base)
            self.assets.discard("USDT")
            
            logger.info(f"[Scanner CACHE] Analyzing {len(self.assets)} unique assets")
            logger.info("[Scanner CACHE] Fetching ALL prices in one call (via fetch_tickers)...")
            
            start_time = time.time()
            
            # CRITICAL FIX: Use fetch_tickers() - ONE call for all prices (~400ms)
            # Instead of fetch_ticker() loop which takes ~100 seconds
            try:
                all_tickers = self.exchange.fetch_tickers(self.usdt_pairs)
                ...
                for symbol in self.usdt_pairs:
                    try:
                        ...
                        if bid > 0 and ask > 0:
                            self.pair_cache[symbol] = PairInfo(
                                symbol=symbol,
                                base=base,
                                quote=quote,
                                bid=bid,
                ...
                logger.info(f"[Scanner CACHE] Loaded {loaded} pairs in {elapsed:.2f}s")
                logger.info("[Scanner CACHE] Cache ready for fast arbitrage detection")
                
            except Exception as e:
                logger.error(f"[Scanner CACHE] fetch_tickers failed: {e}")
                logger.error("Falling back to individual fetches...")
                raise
            
        except Exception as e:
            logger.error(f"[Scanner CACHE] Failed to initialize: {e}")
            logger.error(traceback.format_exc())

    async def update_prices(self):
        """
        Update prices using fetch_tickers() - ONE call for all prices (~400ms).
        Runs every 5 seconds to keep prices fresh.
        """
        try:
            if not self.cache_loaded:
                return
            
            # CRITICAL FIX: fetch_tickers() instead of looping fetch_ticker()
            all_tickers = self.exchange.fetch_tickers(self.usdt_pairs)
            updated = 0
            ...
            for symbol in self.usdt_pairs:
                try:
                    ...
                    if bid > 0 and ask > 0:
                        ...
                        if symbol in self.pair_cache:
                            self.pair_cache[symbol].bid = bid
                            self.pair_cache[symbol].ask = ask
            ...
            if updated > 0:
                logger.debug(f"[Scanner PRICE] Updated {updated} prices via fetch_tickers")
            
        except Exception as e:
            logger.error(f"[Scanner PRICE] Update failed: {e}")

    async def detect_arbitrage_fast(self) -> List[Dict]:
        """
        Fast arbitrage detection with CORRECT triangle math.
        
        Triangle: USDT -> Asset1 -> Asset2 -> USDT
        
        CORRECT formula (many bots get this wrong):
        Step 1: Buy Asset1 with USDT at ASK price
            asset1_amt = USDT / ask(Asset1/USDT)
        Step 2: Sell Asset1 for Asset2 at BID price  
            asset2_amt = asset1_amt * bid(Asset2/Asset1)
        Step 3: Sell Asset2 for USDT at BID price
            final_usdt = asset2_amt * bid(Asset2/USDT)
        
        Fee: 0.1% per trade × 3 trades = 0.3% total
        Min profit must be > 0.3% after fees
        """
        try:
            if not self.cache_loaded:
                return []
            ...
            total_fee = (1 - fee_rate) ** 3  # Three trades with fees
            ...
            for asset1 in assets:
                for asset2 in assets:
                    ...
                    # Required pairs for triangle
                    pair1_sym = f"{asset1}/USDT"  # Buy asset1 with USDT
                    pair2_sym = f"{asset2}/{asset1}"  # Buy asset2 with asset1
                    pair3_sym = f"{asset2}/USDT"  # Sell asset2 for USDT
                    
                    # Get prices from cache
                    p1 = self.price_cache.get(pair1_sym)
                    p2 = self.price_cache.get(pair2_sym)
                    p3 = self.price_cache.get(pair3_sym)
                    
                    # All pairs must exist with valid prices
                    if not (p1 and p2 and p3):
                        continue
                    if not (p1.get("ask") and p2.get("bid") and p3.get("bid")):
                        continue
                    
                    # CORRECT triangle calculation
                    start_usdt = 1.0
                    
                    # Step 1: Buy asset1 at ASK price (we pay the ask)
                    asset1_amount = (start_usdt / p1["ask"]) * (1 - fee_rate)
                    
                    # Step 2: Sell asset1 for asset2 at BID price (we receive the bid)
                    asset2_amount = (asset1_amount * p2["bid"]) * (1 - fee_rate)
                    
                    # Step 3: Sell asset2 for USDT at BID price (we receive the bid)
                    final_usdt = (asset2_amount * p3["bid"]) * (1 - fee_rate)
                    
                    # Calculate profit percentage
                    profit_percent = ((final_usdt - start_usdt) / start_usdt) * 100
                    
                    # Only report if profitable (accounting for 0.3% fees)
                    if profit_percent >= self.min_profit_percent:
                        ...
                        opp = {
                            ...
                            "pairs": [
                                self.pair_cache.get(pair1_sym),
                                self.pair_cache.get(pair2_sym),
                                self.pair_cache.get(pair3_sym)
                            ],
                            "steps": [
                                {"symbol": pair1_sym, "side": "buy", "price": p1["ask"]},
                                {"symbol": pair2_sym, "side": "sell", "price": p2["bid"]},
                                {"symbol": pair3_sym, "side": "sell", "price": p3["bid"]}
                            ]
                        }
                        opportunities.append(opp)
                        logger.info(f"[OPPORTUNITY] {path} = {profit_percent:.6f}%")
            
            # Sort by profit (highest first)
            opportunities.sort(key=lambda x: x["profit_percent"], reverse=True)
            
            logger.debug(f"[Scanner] Checked {triangles_checked} triangles, found {len(opportunities)} profitable")
            
            return opportunities
            
        except Exception as e:
            logger.error(f"[Scanner ARBITRAGE] Detection failed: {e}")
            logger.error(traceback.format_exc())
            return []

    async def scan(self, exchange) -> List[Dict]:
        """
        Main scan method - uses cached data for fast arbitrage detection.
        
        First time: Initializes cache (2-3 minutes)
        Every subsequent scan: Just updates prices and checks for triangles (< 1 second)
        """
        try:
            ...
            if not self.cache_loaded:
                logger.info("[Scanner] First scan - initializing cache...")
                logger.warning("[Scanner] This will take 2-3 minutes for initial setup")
                await self.initialize_cache()
            else:
                # Fast path: Just update prices, don't rescan structure
                logger.info("[Scanner] Cache hit - updating prices only...")
                await self.update_prices()
            
            # Run fast arbitrage detection on cached data
            logger.info("[Scanner] Running arbitrage detection...")
            opportunities = await self.detect_arbitrage_fast()
            
            if opportunities:
                logger.info(f"[Scanner] Found {len(opportunities)} opportunities")
                for opp in opportunities[:5]:
                    logger.info(f"  {opp['path']}: {opp['profit_percent']:.6f}%")
            else:
                logger.debug("[Scanner] No opportunities in this scan")
...
class AutomatedArbitrageBot:
    ...
    def __init__(self, exchange: ccxt.bybit, min_profit_percent: float = 0.6):
        self.exchange = exchange
        self.scanner = ArbitrageScanner(exchange, min_profit_percent)
        self.executor = TradeExecutor(exchange)
        self.running = False
        # Fast scanning: cache hits run in <1 second, so can scan every 5 seconds
        # Initial cache setup takes 2-3 minutes but only happens once
