...
class ArbitrageScanner:
    ...
    def __init__(self, exchange, min_profit_percent: float = 0.1):
    ...
    async def initialize_cache(self):
        """
        Load all pairs once and build caches.
        This runs once at startup - then we only update prices.
        """
        try:
            logger.info("[Scanner CACHE] Initializing pair cache (one-time load)...")
            logger.info("[Scanner CACHE] This may take 2-3 minutes but only runs once...")
            
            self.all_symbols = self.exchange.symbols
            self.usdt_pairs = [s for s in self.all_symbols if "/USDT" in s]
            
            logger.info(f"[Scanner CACHE] Total symbols: {len(self.all_symbols)}")
            logger.info(f"[Scanner CACHE] USDT pairs: {len(self.usdt_pairs)}")
            ...
            logger.info(f"[Scanner CACHE] Unique assets: {len(self.assets)}")
            logger.info("[Scanner CACHE] Preloading prices for all pairs...")
            
            # Fetch initial prices for all USDT pairs
            loaded = 0
            failed = 0
            for i, symbol in enumerate(self.usdt_pairs):
                try:
                    ...
                    loaded += 1
                    if (i + 1) % 200 == 0:
                        logger.info(f"[Scanner CACHE] Preloaded {i + 1}/{len(self.usdt_pairs)} pairs...")
                    
                except Exception as e:
                    failed += 1
                    logger.debug(f"[Scanner CACHE] Failed to load {symbol}: {e}")
                    continue
            
            self.cache_loaded = True
            self.last_cache_update = time.time()
            
            logger.info("[Scanner CACHE] Cache initialization complete!")
            logger.info(f"[Scanner CACHE] Loaded: {loaded} pairs, Failed: {failed} pairs")
            logger.info("[Scanner CACHE] Ready for fast arbitrage detection")
    ...
    async def update_prices(self):
        """
        Update prices for cached pairs quickly (without rescanning structure).
        This runs every 5 seconds to keep prices fresh.
        """
        try:
            if not self.cache_loaded:
                logger.warning("[Scanner PRICE] Cache not loaded yet")
                return
            
            logger.debug("[Scanner PRICE] Updating prices for cached pairs...")
            updated = 0
            failed = 0
            
            for symbol in self.usdt_pairs[:500]:  # Update top 500 pairs (most liquid)
                try:
                    ticker = self.exchange.fetch_ticker(symbol)
                    ...
                    if symbol in self.pair_cache:
                        self.pair_cache[symbol].bid = ticker.get("bid", 0)
                        self.pair_cache[symbol].ask = ticker.get("ask", 0)
            ...
            if updated > 0:
                logger.debug(f"[Scanner PRICE] Updated {updated} prices, failed {failed}")
    ...
    async def detect_arbitrage_fast(self) -> List[Dict]:
        """
        Fast arbitrage detection using cached pair data.
        Checks cached pairs against updated prices - NO rescanning needed.
        """
        try:
            if not self.cache_loaded:
                logger.warning("[Scanner ARBITRAGE] Cache not loaded, skipping detection")
                return []
            ...
            # Check all combinations quickly against cached data
            triangles_checked = 0
            profitable = 0
            ...
            for i, asset1 in enumerate(assets):
                for asset2 in assets:
                    ...
                    pair1_sym = f"{asset1}/USDT"
                    pair2_sym = f"{asset2}/{asset1}"
                    pair3_sym = f"{asset2}/USDT"
                    ...
                    # All pairs must exist and have valid prices
                    if not (p1 and p2 and p3 and p1.get("ask") and p2.get("ask") and p3.get("bid")):
                        continue
                    
                    # Simulate arbitrage
                    start = 1.0
                    step1 = (start / p1["ask"]) * (1 - fee_rate)
                    step2 = (step1 * p2["ask"]) * (1 - fee_rate)
                    step3 = (step2 * p3["bid"]) * (1 - fee_rate)
                    
                    profit = ((step3 - start) / start) * 100
                    ...
                    if profit >= self.min_profit_percent:
                        ...
                        opp = {
                            ...
                            "steps": [
                                ...
                                {"symbol": pair2_sym, "side": "buy", "price": p2["ask"]},
                                {"symbol": pair3_sym, "side": "sell", "price": p3["bid"]}
                            ]
                        }
                        opportunities.append(opp)
                        logger.info(f"[OPPORTUNITY] {path} = {profit:.6f}% profit")
            
            # Sort by profit (highest first)
            opportunities.sort(key=lambda x: x["profit_percent"], reverse=True)
            
            logger.info(f"[Scanner ARBITRAGE] Checked {triangles_checked} triangles in {len(assets)} assets")
            logger.info(f"[Scanner ARBITRAGE] Found {len(opportunities)} profitable opportunities")
...
class AutomatedArbitrageBot:
    ...
    def __init__(self, exchange: ccxt.bybit, min_profit_percent: float = 0.1):
        self.exchange = exchange
