...
class ArbitrageScanner:
    ...
    async def get_all_usdt_pairs(self) -> List[PairInfo]:
        """Fetch all USDT trading pairs from Bybit (with optimization)"""
        try:
            logger.info("[Scanner] Discovering USDT pairs...")
            markets = self.exchange.symbols
            
            # Filter to USDT pairs only
            usdt_symbols = [s for s in markets if "/USDT" in s]
            logger.info(f"[Scanner] Total USDT pairs available: {len(usdt_symbols)}")
            
            # Limit to first 50 pairs for faster scanning (most volume is in top pairs)
            usdt_symbols = usdt_symbols[:50]
            logger.info(f"[Scanner] Scanning top {len(usdt_symbols)} pairs for arbitrage")
            ...
            for i, symbol in enumerate(usdt_symbols):
                try:
                    ...
                    if (i + 1) % 10 == 0:
                        logger.debug(f"[Scanner] Loaded {i + 1}/{len(usdt_symbols)} pairs...")
                    
                except Exception as e:
                    logger.debug(f"Failed to fetch {symbol}: {e}")
                    continue
            
            logger.info(f"[Scanner] Successfully loaded {len(usdt_pairs)} USDT pairs")
            return usdt_pairs
        except Exception as e:
    ...
    async def find_triangle_arbitrage(self, pairs: List[PairInfo]) -> List[Dict]:
        """Find triangle arbitrage opportunities (USDT -> Base -> Quote -> USDT)"""
        try:
            logger.info("[Scanner] Analyzing for triangle arbitrage opportunities...")
            opportunities = []
            
            # Organize pairs by base asset
            ...
            # Get all unique bases (excluding USDT)
            bases = [b for b in pairs_by_base.keys() if b != "USDT"]
            logger.info(f"[Scanner] Checking {len(bases)} base assets for triangles")
