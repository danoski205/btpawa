    ...
    def _init_exchange(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Bybit API test response: {response.status_code}")
                if response.status_code != 200:
                    logger.warning(f"Bybit API returned {response.status_code} - may indicate IP blocking or network issue")
            except Exception as api_test_error:
                logger.error(f"Cannot reach Bybit API (IP may be blocked): {api_test_error}")
            
            # Initialize exchange
            logger.info(f"Initializing ccxt.bybit with USE_TESTNET={USE_TESTNET}")
            ...
            for attempt in range(3):
                try:
                    logger.info(f"load_markets attempt {attempt + 1}/3...")
                    self.exchange.load_markets()
                    logger.info("load_markets succeeded!")
                    break
